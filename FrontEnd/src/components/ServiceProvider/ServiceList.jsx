    ...
    const fetchData = async () => {
      try {
        const result = await axios.get(
          ...
          // "http://localhost:7373/serviceprovider/getall" + sessionStorage["skills"]
        );
        ...
        console.log("error occurreddddd", err);
      }
    };
        ...
        <tbody>
          {slist.map((ob, index) => (
            <tr key={index}>
              ...
              <td>{ob.address}</td>
              <td>{ob.date}</td>
              {/* <input type='checkbox' onChange={()=>handleUser(ob.userId)}></input> */}
              <td>
